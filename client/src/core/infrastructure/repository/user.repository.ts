...
export class UserRepository implements UserRepositoryInterface {
  async getAllFacultyData(params: PaginationParams<User>): Promise<PaginatedUserResponseDTO> {
    const { data } = await api.get(API_CONFIG.enpoints.USER.FACULTY.GET, {
      params: params,
    });
    return data;
  }
  ...
  async getAllStudentData(params: PaginationParams<User>): Promise<PaginatedUserResponseDTO> {
    const { data } = await api.get(API_CONFIG.enpoints.USER.STUDENT.GET, {
      params: params,
    });
  ...
  async getFacultyByID(id: number): Promise<UserSuccessDTO> {
    const { data } = await api.get(`${API_CONFIG.enpoints.USER.FACULTY.GET}/${id}`);
  ...
  async addFaculty(facultyData: FacultyRequest): Promise<UserSuccessDTO> {
    ...
    const { data } = await api.post(API_CONFIG.enpoints.USER.FACULTY.ADD, payload, {
      headers: {
  ...
  async editFaculty(facultyData: FacultyRequest, facultyId: number): Promise<UserSuccessDTO> {
    ...
    const { data } = await api.post(`${API_CONFIG.enpoints.USER.FACULTY.ADD}/${facultyId}`, payload, {
      headers: {
        Accept: "application/json",
