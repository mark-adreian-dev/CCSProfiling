...
export default function FacultyForm({ targetID, FormTrigger }: FacultyFormProps) {
  ...
  useEffect(() => {
    ...
    if (isEdit && data?.data) {
      ...
      form.reset({
        ...
        profile_picture: faculty.profile_picture, // Keep null unless you want to handle existing image previews
      });
    }
  ...
  return (
    <Form<FacultyRequest>
      ...
      trigger={<FormTrigger />}
      className="w-300!"
      open={isOpen} // 3. Pass state
      onOpenChange={setIsOpen} // 4. Pass setter
    >
